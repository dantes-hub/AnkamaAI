...
// ---- Files ----
export async function listFiles(tenantId, projectId) {
  const url = `${BASE}/files?tenant_id=${encodeURIComponent(tenantId)}&project_id=${encodeURIComponent(projectId)}`;
  const res = await fetch(url, { headers: authHeaders() });
  if (!res.ok) throw new Error(`files failed: ${res.status} ${await res.text()}`);
  return res.json();
}

export async function deleteFile(fileId, tenantId, projectId) {
  const url = `${BASE}/files/${encodeURIComponent(fileId)}?tenant_id=${encodeURIComponent(tenantId)}&project_id=${encodeURIComponent(projectId)}`;
...
export async function ingest(formData, tenant='demo-tenant', project='kb') {
  const url = `${BASE}/ingest?tenant_id=${encodeURIComponent(tenant)}&project_id=${encodeURIComponent(project)}`;
...
export async function ask(query, opts = {}) {
  const { tenant_id='demo-tenant', project_id='kb', user_id='demo-user', top_k=8 } = opts;
  const body = { tenant_id, project_id, user_id, query, top_k };
  const res = await fetch(`${BASE}/ask`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', ...authHeaders() },
    body: JSON.stringify(body)
  });
  if (!res.ok) throw new Error(`ask failed: ${res.status} ${await res.text().catch(()=> '')}`);
  return res.json();
}
...
export function streamChatES(messages, { model } = {}) {
  ...
  if (AUTH_TOKEN) qs.set('access_token', AUTH_TOKEN); // EventSource can't set headers
  const es = new EventSource(`${BASE}/chat/sse?${qs.toString()}`);
  ...
  es.onmessage = (ev) => {
    try {
      ...
      if (data.done || data.error) { handlers.done?.(data); es.close(); }
    } catch {}
  };
